...
export const ModalTrigger = ({ className }: { className?: string }) => {
  ...
  return (
    <button
      className={cn(
        "bg-secondary relative z-10 hover:bg-secondary/90 group hover:-translate-y-0.5 active:scale-[0.98] text-black text-sm md:text-sm transition font-medium duration-200  rounded-md px-4 py-2  flex items-center justify-center scale-125",
        className
      )}
...
export const SmallModalTrigger = ({ className }: { className?: string }) => {
  const { setOpen } = useModal()
  return (
    <button
      className={cn(
